...
class DetailDepositoUserPage extends StatelessWidget {
  ...
  Future<Map<String, String>> _loadDepositoData() async {
    ...
    return {
      ...
      'deposit_name': storage.read('deposit_name') ?? '',
      'amount': storage.read('amount') ?? '',
      'min_amount': storage.read('min_amount') ?? '',
    };
  }
  ...
  void _deleteDeposito(Map<String, String> depositoDetail, BuildContext context) {
    final storage = GetStorage();
    storage.remove('deposit_name');
    storage.remove('amount');
    storage.remove('min_amount');
